...
from test_new_gimbal_4_debug import (
    pixel_to_latlon
)
...
for zone_id, center_row, center_col, area in zone_centers_cc:
    ...
    result = pixel_to_latlon(
        ...
        gimbal_yaw=0,  # Adjust based on gimbal orientation
        gimbal_pitch=-89.9,  # Nearly straight down
        gimbal_roll=0,
        debug=False
    )
    print(f"Zone {zone_id} GPS: Latitude {result['lat']:.6f}, Longitude {result['lon']:.6f}")
